...
class StaticFiles:
    ...
    def get_directories(
        ...
        for package in packages or []:
            ...
            assert spec is not None, "Package {} could not be found.".format(
                repr(package)
            )
            assert (
    ...
    async def check_config(self) -> None:
        ...
        try:
            ...
            raise RuntimeError(
                "StaticFiles directory '{}' does not exist.".format(self.directory)
            )
        if not (stat.S_ISDIR(stat_result.st_mode) or stat.S_ISLNK(stat_result.st_mode)):
            raise RuntimeError(
                "StaticFiles path '{}' is not a directory.".format(self.directory)
            )
